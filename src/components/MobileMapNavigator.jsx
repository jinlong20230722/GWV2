  ...
  return <div className={`${height} w-full relative rounded-2xl overflow-hidden bg-[#2D3748]`}>
      ...
      <div className="relative z-10 h-full flex flex-col items-center justify-center bg-gradient-to-br from-[#0A1628]/90 via-[#2D3748]/80 to-[#0A1628]/90">
        <div className="text-center px-4">
          ...
          {!showOptions ?
        // 主导航按钮
        <button onClick={() => setShowOptions(true)} className="inline-flex items-center space-x-2 px-8 py-3 bg-gradient-to-r from-[#D4AF37] to-[#C0C0C0] text-[#0A1628] rounded-full font-semibold hover:opacity-90 transition-all duration-300 transform hover:scale-105 shadow-lg">
        ...
        <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button onClick={() => openMapApp('amap')} className="flex flex-col items-center justify-center px-4 py-3 bg-[#2D3748] border border-[#2D3748] hover:border-[#D4AF37] rounded-xl transition-all duration-300 transform hover:scale-105">
                  <Smartphone className="w-6 h-6 text-[#D4AF37] mb-2" />
                  <span className="text-white text-sm font-medium">高德地图</span>
                </button>
                
                <button onClick={() => openMapApp('qqmap')} className="flex flex-col items-center justify-center px-4 py-3 bg-[#2D3748] border border-[#2D3748] hover:border-[#D4AF37] rounded-xl transition-all duration-300 transform hover:scale-105">
                  <Smartphone className="w-6 h-6 text-[#D4AF37] mb-2" />
                  <span className="text-white text-sm font-medium">腾讯地图</span>
                </button>
                
                <button onClick={() => openMapApp('baidu')} className="flex flex-col items-center justify-center px-4 py-3 bg-[#2D3748] border border-[#2D3748] hover:border-[#D4AF37] rounded-xl transition-all duration-300 transform hover:scale-105">
                  <Globe className="w-6 h-6 text-[#D4AF37] mb-2" />
                  <span className="text-white text-sm font-medium">百度地图</span>
                </button>
              </div>
              
              <button onClick={() => setShowOptions(false)} className="text-gray-400 text-sm hover:text-white transition-colors mt-2">
                返回
              </button>
            </div>}
        </div>
